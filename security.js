...
/**
 * Toggles the manual S.O.S. state.
 * @returns {boolean} The new state of the S.O.S. signal (true if active).
 */
export function toggleManualSOS() {
    manualSOS = !manualSOS;
    // If we are deactivating, reset the last position to avoid false "sudden stop" alerts.
    if (!manualSOS) {
        lastPosition = null;
    }
    return manualSOS;
...
/**
 * Gets the current manual S.O.S. state without changing it.
 * @returns {boolean} True if manual S.O.S. is active.
 */
export function isManualSOSActive() {
    return manualSOS;
}
